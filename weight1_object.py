				...
				if hecke_used.count(h) == 0:
					...
					for S in self.spaces():
						if S.unique(): #! why this as opposed to just h unique in S?
							...
							lb = multiplicity(forms,h,Qchi)
							# dp = max_degree(hp)
							self.output(4,"    have "+str(modp_mult)+" copies of this form mod "+str(p)+" and am expecting at least "+str(lb))
							# second condition below can be tripped if CM form is already removed via a congruence
							assert (modp_mult >= lb) or (modp_mult == 0),"CM/old not found!!"+str(f)+str(self)
							if modp_mult == lb:
...
# forms is a list generated by oldspan of tuples (f,phi,m)
# function returns the number of such tuples where the min polys of f match h (weighted by m)
def multiplicity(forms,h,Qchi):
	ans = 0
	for F in forms:
		f = F[0]
		phi = F[1]
		...
		hf = {}
		for q in h.keys():
			hf[q] = [minpoly_over(f[q],Qchi,phi)]
		if hf == h:
			ans += m
	return ans
